                ...
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-header" style="display: flex; justify-content: center;">
                            <h2 class="fw-medium fs-base">Tambah List Bahan CS</h2>
                        </div>

                        <!-- BEGIN: Modal Body -->
                        <form action="{{ route('create.bahan-cs') }}" method="POST">
                            @csrf
                                ...
                                <div class="col-12 col-sm-6">
                                    ...
                                    <select id="category" name="category_id" class="form-select">
                                        ...
                                        @foreach ($categories as $category)
                                            @php
                                                $isBahanCsroute = Route::is('bahan-cs');
                                                $isBahanCsCategory =
                                                    strtolower($category->category_name) === 'bahan cs';
                                            @endphp
                                            @if (($isBahanCsroute && $isBahanCsCategory) || (!$isBahanCsroute && !$isBahanCsCategory))
                    ...
                    <tbody>
                        @foreach ($data as $item)
                            ...
                            <div id="edit-{{ $item->id }}" class="modal fade" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-sm">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('update.bahan-cs', ['id' => $item->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('patch')
                                            <div class="modal-body d-flex justify-content-center gap-2 align-items-center">
                                                <div class="col-12 col-sm-6">
                                                    <label for="modalName" class="form-label">Nama Item</label>
                            ...
                            <div id="itemIn-{{ $item->id }}" class="modal fade" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-sm">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('itemIn.bahan-cs', ['id' => $item->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('patch')
                            ...
                            <div id="itemOut-{{ $item->id }}" class="modal fade" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-sm">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('itemOut.bahan-cs', ['id' => $item->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('patch')
                                            <div class="modal-body d-flex justify-content-center align-items-center">
                            ...
                            <div id="delete-{{ $item->id }}" class="modal fade" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-sm">
                                    <div class="modal-content">
                                        <form action="{{ route('destroy.bahan-cs', ['id' => $item->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('delete')
                                            <div class="modal-body d-flex justify-content-center align-items-center">
        ...
        <div class="row g-4">
            <div class="col-sm-12 col-xl-6">
                <div class="bg-light text-center rounded p-4 h-100">
                    ...
                    <canvas height="250" id="bahanCsChart"></canvas>
                </div>
            </div>
            <div class="col-sm-12 col-xl-6 hidden" id="monthlyContainer">
                <div class="bg-light text-center rounded p-4 h-100">
                    ...
                    <canvas height="250" id="monthBahanCsChart"></canvas>
                </div>
            </div>
            <div class="col-sm-12 col-xl-6">
                <div class="bg-light text-center rounded p-4 h-100">
                    ...
                    <canvas height="250" id="bahanCsStok"></canvas>
                </div>
            </div>
        </div>
    </div>
...
@section('jsPage')
    <script>
        ...
        $(document).ready(function() {
            var ctx1 = $("#bahanCsChart").get(0).getContext("2d");
        ...
        $(document).ready(function() {
            var ctx1 = $("#monthBahanCsChart").get(0).getContext("2d");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            var ctx1 = document.getElementById("bahanCsStok").getContext("2d");
        ...
        document.getElementById('showMonthlyButton').addEventListener('click', function() {
            ...
            document.getElementById('bahanCsChart').parentElement.parentElement.classList.add('hidden');
        });
        ...
        document.getElementById('showDailyButton').addEventListener('click', function() {
            ...
            document.getElementById('bahanCsChart').parentElement.parentElement.classList.remove('hidden');
        });
    </script>
@endsection
