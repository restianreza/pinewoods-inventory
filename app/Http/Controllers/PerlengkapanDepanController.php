...
class PerlengkapanDepanController extends Controller
{
    public function index()
    {
        $categories = Categorie::all();
        $data = Item::with('categorie')->whereHas('categorie', function ($query) {
            $query->where('category_name', 'Perlengkapan Depan');
        })->get();
        $showcaseCategory = Categorie::where('category_name', 'Perlengkapan Depan')->first();

        if (!$showcaseCategory) {
            return view('pages.tim-depan.perlengkapan-depan', compact('categories'))->with('errors', 'Perlengkapan Depan category not found');
        }
        ...
        $riwayat = History::where('action_category', 'Perlengkapan Depan')->whereBetween('created_at', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()])->orderBy('created_at', 'desc')->get();


        return view('pages.tim-depan.perlengkapan-depan', compact('data', 'categories', 'items', 'histories', 'itemNames', 'itemQuantities', 'riwayat', 'monthHistories'));
    }
    ...
    public function create(Request $request)
    {
        ...
        History::create([
            ...
            'action' => "Create",
            'action_category' => 'Perlengkapan Depan',
            'item_id' => $item->id,
    ...
    public function destroy($id)
    {
        ...
        History::create([
            ...
            'before_change' => json_encode($before),
            'after_change' => null,  // Tidak ada state setelah penghapusan
            'user' => auth()->user()->name
