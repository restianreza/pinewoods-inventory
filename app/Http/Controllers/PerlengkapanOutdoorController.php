...
class PerlengkapanOutdoorController extends Controller
{
    public function index()
    {
        $categories = Categorie::all();
        $data = Item::with('categorie')->whereHas('categorie', function ($query) {
            $query->where('category_name', 'Perlengkapan Outdoor');
        })->get();
        $showcaseCategory = Categorie::where('category_name', 'Perlengkapan Outdoor')->first();

        if (!$showcaseCategory) {
            return view('pages.cleaning-service.perlengkapan-outdoor', compact('categories'))->with('errors', 'Perlengkapan Outdoor category not found');
        }
        ...
        $riwayat = History::where('action_category', 'Perlengkapan Outdoor')->whereBetween('created_at', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()])->orderBy('created_at', 'desc')->get();


        return view('pages.cleaning-service.perlengkapan-outdoor', compact('data', 'categories', 'items', 'histories', 'itemNames', 'itemQuantities', 'riwayat', 'monthHistories'));
    }
    ...
    public function create(Request $request)
    {
        ...
        History::create([
            ...
            'action' => "Create",
            'action_category' => 'Perlengkapan Outdoor',
            'item_id' => $item->id,
