...
class DryGoodController extends Controller
{
    public function index()
    {
        ...
        $data = Item::with('categorie')->whereHas('categorie', function ($query) {
            $query->where('category_name', 'Dry Good');
        })->get();
        $showcaseCategory = Categorie::where('category_name', 'Dry Good')->first();

        if (!$showcaseCategory) {
            return view('pages.kitchen.dry-good', compact('categories'))->with('errors', 'Dry Good category not found');
        }
        ...
        $riwayat = History::where('action_category', 'Dry Good')->whereBetween('created_at', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()])->orderBy('created_at', 'desc')->get();

        return view('pages.kitchen.dry-good', compact('data', 'categories', 'items', 'histories', 'itemNames', 'itemQuantities', 'riwayat', 'monthHistories'));
    }
