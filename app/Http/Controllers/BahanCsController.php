...
class BahanCsController extends Controller
{
    public function index()
    {
        $categories = Categorie::all();
        $data = Item::with('categorie')->whereHas('categorie', function ($query) {
            $query->where('category_name', 'Bahan Cs');
        })->get();
        $showcaseCategory = Categorie::where('category_name', 'Bahan Cs')->first();

        if (!$showcaseCategory) {
            return view('pages.cleaning-service.bahan-cs', compact('categories'))->with('errors', 'Bahan Bar category not found');
        }
        ...
        $riwayat = History::where('action_category', 'Bahan Cs')->whereBetween('created_at', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()])->orderBy('created_at', 'desc')->get();


        return view('pages.cleaning-service.bahan-cs', compact('data', 'categories', 'items', 'monthHistories', 'histories', 'itemNames', 'itemQuantities', 'riwayat'));
    }
