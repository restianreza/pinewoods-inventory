...
class FroozenFoodController extends Controller
{
    public function index()
    {
        $categories = Categorie::all();
        $data = Item::with('categorie')->whereHas('categorie', function ($query) {
            $query->where('category_name', 'Froozen Food');
        })->get();
        $showcaseCategory = Categorie::where('category_name', 'Froozen Food')->first();

        if (!$showcaseCategory) {
            return view('pages.kitchen.froozen-food', compact('categories'))->with('errors', 'Froozen Food category not found');
        }
        ...
        $riwayat = History::where('action_category', 'Froozen Food')->whereBetween('created_at', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()])->orderBy('created_at', 'desc')->get();


        return view('pages.kitchen.froozen-food', compact('data', 'categories', 'items', 'histories', 'itemNames', 'itemQuantities', 'riwayat', 'monthHistories'));
    }
