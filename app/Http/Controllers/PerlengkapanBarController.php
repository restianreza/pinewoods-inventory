...
class PerlengkapanBarController extends Controller
{
    public function index()
    {
        $categories = Categorie::all();
        $data = Item::with('categorie')->whereHas('categorie', function ($query) {
            $query->where('category_name', 'Perlengkapan Bar');
        })->get();
        $showcaseCategory = Categorie::where('category_name', 'Perlengkapan Bar')->first();

        if (!$showcaseCategory) {
            return view('pages.bar.perlengkapan-bar', compact('categories'))->with('errors', 'Perlengkapan Bar category not found');
        }
        ...
        $riwayat = History::where('action_category', 'Perlengkapan Bar')->whereBetween('created_at', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()])->orderBy('created_at', 'desc')->get();

        return view('pages.bar.perlengkapan-bar', compact('data', 'categories', 'items', 'histories', 'itemNames', 'itemQuantities', 'riwayat', 'monthHistories'));
    }
    ...
    public function create(Request $request)
    {
        ...
        History::create([
            ...
            'action' => "Create",
            'action_category' => 'Perlengkapan Bar',
            'item_id' => $item->id,
